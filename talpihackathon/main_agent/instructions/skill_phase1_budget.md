...
5. **Collect hierarchical breakdown for flow chart:** Additionally, execute a query to fetch the active budget items (their `code`, `title`, `amount_allocated`, and `level`) for a recent year (e.g., 2025 or 2026) under this subject. This hierarchy data (from level 1 down to level 3 or 4) is required by the synthesis agent to draw the Sankey/Mermaid flow diagram mapping budget sources/ministries to domains and program items.
