...
reproducible enough to compare models against each other.
    ...
    python pipeline.py --subject "אנרגיה ירוקה" --provider vertex
"""
...
try:
    from agent import print_agent, print_error, setup_llm_provider
except Exception:  # pragma: no cover - keeps the pipeline usable standalone
...
# inside the provider, so overshooting degrades rather than fails. Override with
# --max-parallel, or set it to 1 to get the old sequential behaviour back.
...
def step_domains(provider, subject: str, plan: Dict, domains: List[Dict], cost: Cost,
    ...
    verdicts = _judge_batch(
        provider, "triage_domains.md", subject,
...
def _judge_batch(provider, prompt_file: str, subject: str,
...
def step_triage(provider, subject: str, plan: Dict, programs: List[Dict], cost: Cost,
                max_parallel: Optional[int] = None) -> Dict[str, Dict]:
    verdicts = _judge_batch(
        provider, "triage_programs.md", subject,
...
def step_judge(provider, subject: str, plan: Dict, retrieved: Dict,
    ...
    verdicts = _judge_batch(
        provider, "judge_items.md", subject,
...
def build_hierarchy(api: BudgetAPI, out_dir: str, offices: List[str],
    ...
    Replaces skill_phase5_hierarchy.md, which told the model to use
...
def build_digest(out_dir: str, subject: str, top_n: int = 25) -> str:
    ...
    Phases 2, 3 and 5 take Phase 1's findings as free text in their prompt. They need
...
def default_out_dir(subject: str, label: Optional[str]) -> str:
    slug = re.sub(r"[^\w\-]", "_", subject)
    stamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    name = "%s-%s" % (slug, stamp)
    if label:
        name += "-" + re.sub(r"[^\w\-.]", "_", label)
    return os.path.join(os.path.dirname(os.path.abspath(__file__)), "pipeline_runs", name)


def main():
    p = argparse.ArgumentParser(description="Deterministic budget-item discovery pipeline")
    ...
    p.add_argument("--label", default=None,
                   help="Tag for the run directory; defaults to the model name")
    p.add_argument("--mcp-url", default="https://next.obudget.org/mcp")
    p.add_argument("--stop-after", choices=["expand", "domains", "retrieve"],
                   help="Stop early - useful for checking the search plan cheaply")
    p.add_argument("--max-parallel", type=int, default=MAX_PARALLEL_CALLS,
                   help="Concurrent model calls in triage/judging (default %d; 1 = sequential)"
                        % MAX_PARALLEL_CALLS)
    args = p.parse_args()

    if setup_llm_provider is None:
        print_error("could not import setup_llm_provider from agent.py")
        sys.exit(1)
    provider = setup_llm_provider(args.provider, args.model)
    model_name = getattr(provider, "model", None) or getattr(provider, "cmd", "unknown")
    out_dir = args.out or default_out_dir(args.subject, args.label or model_name)
    os.makedirs(out_dir, exist_ok=True)
    print_agent("Output: %s" % out_dir)

    mcp = MCPClient(args.mcp_url)
    mcp.connect()
    mcp.initialize()
    try:
        run_pipeline(args.subject, out_dir, provider, mcp, args.stop_after,
                     args.max_parallel)
    finally:
        mcp.close()
